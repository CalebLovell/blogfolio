...
export const TechList = () => {
	return (
		<div className='w-full space-y-6'>
			...
			<ul className='grid grid-cols-2 text-red-400 sm:grid-cols-4 md:grid-cols-5 gap-x-4 gap-y-4'>
				{backend.map(x => (
					<div key={x.title} className='flex flex-col items-center justify-center w-full h-full px-1 py-4 bg-gray-800 rounded-md transition duration-75 ease-in-out hover:scale-110 hover:bg-gray-700'>
						{x.icon}
						<p className='mt-2 font-semibold text-center text-gray-200'>{x.title}</p>
					</div>
				))}
